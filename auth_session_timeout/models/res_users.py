...
class ResUsers(models.Model):
    ...
    @api.model
    def _auth_timeout_check(self, endpoint=None):
        ...
        if terminated:
            current_endpoint = endpoint if endpoint is not None else http.request.endpoint
            is_json = current_endpoint and current_endpoint.routing['type'] == 'json'
            if is_json:
                # Para peticiones JSON (como el carrito de compras)
                # Esto le dice al JavaScript de Odoo que recargue la página completa.
                raise SessionExpiredException("Session expired")
            else:
                redirect_url = '/web/login?redirect=%s' % http.request.httprequest.full_path
                raise HTTPException(response=redirect(redirect_url, code=302))
