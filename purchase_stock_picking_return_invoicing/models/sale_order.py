...
class PurchaseOrderLine(models.Model):
    ...
    qty_refunded = fields.Float(compute="_compute_qty_refunded", string="Reembolsado")
    ...
    def _compute_qty_refunded(self):
        for line in self:
            inv_lines = line.invoice_lines.filtered(
                lambda x: (
                    (x.move_id.move_type == "out_invoice" and x.quantity < 0.0)
                    or (x.move_id.move_type == "out_refund" and x.quantity > 0.0)
                )
            )
    ...
    def _compute_qty_returned(self):
        ...
        groups = self.env["stock.move"].read_group(
            ...
            ["sale_line_id", "product_uom_qty", "product_uom"],
        ...
        for g in groups:
            ...
            line = self.browse(g["sale_line_id"][0])
    ...
    #     if (move and move.move_type == "out_refund") or (
    #             not move and move_type and move_type == "out_refund"
    #     ):
    #         data["quantity"] *= -1.0
    #     return data
