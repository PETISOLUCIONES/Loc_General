...
class AccountPayment(models.Model):
    ...
    advance = fields.Boolean('Advance Payment?', default=False)
    advance_payment_account_id = fields.Many2one('account.account', 'Advance Payment Account')
    residual = fields.Monetary(string='Remaining Amount', 
    ...
    move_reconciled = fields.Boolean(compute="_compute_move_reconciled", 
    readonly=True)
    ...
    def _compute_destination_account_id(self):
        ...
        for payment in self:
            if (payment.payment_type != 'transfer' and payment.advance):
                company = self.journal_id.company_id
                if self.payment_type == 'inbound':
                    advance_payment_account_id = company.advance_payment_account_id.id
                else:
                    advance_payment_account_id = company.advance_payment_outgoing_account_id.id
                payment.destination_account_id = advance_payment_account_id

    @api.depends('advance')
    def _compute_partner_id(self):
        super(AccountPayment, (self.filtered(lambda x: not x.advance)))._compute_partner_id()
    ...
    def _compute_residual(self):
        ...
        for payment in (self - reconciled_payment):
            ...
            for line in invoice_lines.filtered('advance_account'):
    ...
    #     self.advance = self.env.context.get('default_advance', False)
    ...
    def _seek_for_lines(self):
        ...
        if self.advance and self.destination_account_id:
            counterpart_lines.advance_account = True
        else:
            counterpart_lines.advance_account = False
