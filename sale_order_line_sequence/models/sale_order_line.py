...
class SaleOrderLine(models.Model):
    ...
    sequence2 = fields.Integer(
        string="Número",
        related="visible_sequence",
        readonly=True,
    )

    @api.depends("sequence", "order_id.order_line.sequence", "order_id.order_line.display_type")
    def _compute_visible_sequence(self):
        for so in self.mapped("order_id"):
            ...
            for line in sorted(order_lines, key=lambda order_line: (order_line.sequence, order_line.id or 0)):
                line.visible_sequence = sequence
                sequence += 1
        for line in self:
            if line.display_type or not line.order_id:
                line.visible_sequence = 0
