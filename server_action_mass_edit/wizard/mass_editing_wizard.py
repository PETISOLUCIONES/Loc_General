...
class MassEditingWizard(models.TransientModel):
    ...
    def onchange(self, values, field_names, fields_spec):
        """
        Versión optimizada para Odoo 19:
        Filtra campos virtuales para evitar KeyErrors en el ORM y
        mantiene la reactividad en la interfaz OWL.
        """
        server_action_id = self.env.context.get("server_action_id")
        server_action = self.env["ir.actions.server"].sudo().browse(server_action_id)

        # 1. Manejo de la llamada inicial (cuando se abre el wizard)
        ...
        # 2. FILTRADO PARA EL ORM (Evita el KeyError)
        # Separamos lo que Odoo conoce (campos reales en Python)
        # de lo que es dinámico (inyectado vía fields_get/arch)
        real_values = {k: v for k, v in values.items() if k in self._fields}
        real_field_names = [f for f in field_names if f in self._fields]
        real_fields_spec = {k: v for k, v in fields_spec.items() if k in self._fields}

        # 3. Ejecución del onchange estándar
        # Pasamos solo los campos reales para que el super() no falle
        res = super().onchange(real_values, real_field_names, real_fields_spec)

        # 4. RE-INYECCIÓN DE DATOS DINÁMICOS
        # Odoo 19/OWL necesita que devolvamos los valores de los campos dinámicos
        # para que no se limpien en la vista después del RPC call.
        if "value" not in res:
            res["value"] = {}

        # Identificamos los campos que son parte de la edición masiva
        mass_edit_fields = []
        if server_action:
            mass_edit_fields = server_action.mapped("mass_edit_line_ids.field_id.name")

        for key, value in values.items():
            # Si el campo es un selector (selection__...) o un campo destino de la edición
            if key.startswith("selection__") or key in mass_edit_fields:
                # Solo lo añadimos si no estaba ya procesado por el super (campos virtuales)
    ...
    def fields_get(self, allfields=None, attributes=None):
        ...
        model_obj = self.env[server_action.model_id.model]
        fields_info = model_obj.fields_get()

        for line in server_action.mapped("mass_edit_line_ids"):
            field = line.field_id
            if field.name not in fields_info:
                continue

            field_info = fields_info[field.name].copy()

            field_info['readonly'] = False

            field_info = self._clean_check_company_field_domain(
                model_obj, field, field_info
            )
