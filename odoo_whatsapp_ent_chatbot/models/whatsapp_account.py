    ...
    def _process_messages(self, value):
        ...
        for messages in value.get("messages", []):
            ...
            if message_type == "interactive":
                ...
                _logger.info('[odoo_whatsapp_ent_chatbot/models/whatsapp_account.py]')
                _logger.info('timestamp: %s', messages['timestamp'])

                if message_type == "interactive":
                    if messages.get("interactive") and messages.get("interactive").get("button_reply"):
                        message = (messages.get("interactive").get("button_reply").get("title"))
                    elif messages.get("interactive") and messages.get("interactive").get("list_reply"):
                        message = (messages.get("interactive").get("list_reply").get("title"))
                    else:
                        message = ""
                    kwargs["body"] = message
