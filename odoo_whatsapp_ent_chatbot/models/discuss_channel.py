...
class ChatbotDiscussChannel(models.Model):
    ...
    script_sequence = fields.Integer(
        string="Sequence",
        default=1
    )

    is_chatbot_ended = fields.Boolean(
        string="Inactivate Chatbot"
    )
    ...
    def _notify_thread(self, message, msg_vals=False, **kwargs):
        res = super(ChatbotDiscussChannel, self)._notify_thread(message, msg_vals=msg_vals, **kwargs)
            ...
            if wa_account_id and user_partner and wa_account_id.wa_chatbot_id and self:
                ...
                if not self.is_chatbot_ended:
                    message_script = (
                        ...
                        .filtered(lambda l: l.message == tools.html2plaintext(mail_message_id.body))
                    )
